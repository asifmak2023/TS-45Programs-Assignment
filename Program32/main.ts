...
console.clear();
const current_users : string[] =  ["admin", "jalil", "kashif", "asif", "tahira", "hussain"];
const new_users : string[] =  ["admin", "ASIF", "shakir", "talha", "liaquat", "jameel"];

for ( let i = 0; i<new_users.length; i++){
  if(current_users.includes(new_users[i].toLowerCase()) || current_users.includes(new_users[i].toUpperCase())){
    console.log(new_users[i] + " is not available. Enter a new user name!");
  }
  else{
