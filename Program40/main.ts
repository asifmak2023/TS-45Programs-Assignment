...
function makeAlbum(artist: string, title: string, tracks?: number): { artist: string; title: string; tracks?: number } {
    const album = {
        artist: artist,
        title: title,
        ...(tracks !== undefined && { tracks: tracks }), // Add tracks only if provided
    };
...
const album1 = makeAlbum("asif", "Title 1", 1);
const album2 = makeAlbum("Amir", "Title 2");
const album3 = makeAlbum("Javed", "Title 3", 3);
